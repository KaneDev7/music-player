...
const music_player_list_items = document.querySelector(
...
const music_player_progressBar = document.querySelector(
...
const toogleShowList = () => {
  ...
  const music_player_list_class = `music_player_list ${
    isShowList && 'showList'
  }`
...
const loadAudio = ({ artist, title, uri, coverart, background, isPlaying }) => {
  ...
  setTimeout(() => {
    music_player_loader.style.opacity = '1'
  }, 1500)
...
const playSong = isPlaying => {
  ...
  if (!isPlaying) {
    list_items[currentIndex].querySelector(
      '.playing_indicator'
  ...
  const update = () => {
    ...
    for (let i = 0; i < bufferLength; i++) {
      ...
      elements[i].style.transform = `rotateZ(${
        i * (360 / bufferLength)
      }deg) translate(-50%, ${clamp(item, 126, 132)}px)`
    }
...
const circle = document.getElementById('smokyCircle')
...
function createSmokeParticle () {
  ...
  particle.style.animation = `rise ${animationDuration}s ease-out forwards`
...
function createSmoke () {
  for (let i = 0; i < 3; i++) {
    setTimeout(createSmokeParticle, i * 100)
  }
}
